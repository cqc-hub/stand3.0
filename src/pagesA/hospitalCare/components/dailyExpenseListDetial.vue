<template>
    ...
    <view class="container">
      <view class="top">
        <view class="title" v-if="costType == '1'">
          {{ costInfoDetal.hosName }}日费用清单
        </view>
        <view class="title" v-if="costType == '3'">
          {{ costInfoDetal.hosName }}费用结算单
        </view>
        <view class="information">
          ...
          <view
            class="item"
            v-if="
              param.isHosTotallist &&
              (costInfoDetal.inDay || costInfoDetal.endDay)
            "
          >
            ...
            <view class="item-content">
              ...
              <text v-if="costInfoDetal.inDay && costInfoDetal.inDay">~</text>
              <text>
          ...
          <view class="item">
            ...
            <view class="item-content">
              {{ costInfoDetal.hospitalWard }}
              {{ costInfoDetal.inpatientBed }}床
            </view>
          </view>
          ...
          <view class="item">
            <view @click="isShowBtn(index)" class="item-top">
              ...
              <view class="money text-no-wrap">
                ...
                <text v-else style="color: #888888" class="iconfont">
                  &#xe6c5;
                </text>
              </view>
            </view>
  ...
  <view class="empty-box" v-else>
    <g-empty
      v-if="props.isHosDaylist"
      :current="2"
      text="当天未产生住院费用,请选择其他日期"
    />
    <g-empty
      v-if="props.isHosTotallist"
      :current="2"
      text="未查询到总计清单信息"
    />
  </view>
...
<script setup lang="ts">
  ...
  const init = async (opt = {}) => {
    Obj.value = undefined;
    pageProps.value = deQueryForUrl<inHospitalCostInfoParam>(
      deQueryForUrl(props)
    );
    param.value = {
      ...pageProps.value,
