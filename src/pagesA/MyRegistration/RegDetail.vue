<template>
  <view :class="{
      ...
      <view v-if="orderRegInfo.patientId" class="box">
        <view class="reg-header flex-between">
          <view :style="{
            ...
            <view v-if="titleStatus.headerIcon.includes('#xe6ea')" class="iconfont reg-header-icon">
              &#xe6ea;
            </view>

            <view v-else-if="titleStatus.headerIcon.includes('#xe6c7')" class="iconfont reg-header-icon">
              &#xe6c7;
            </view>

            <view v-else-if="titleStatus.headerIcon.includes('#xe6d5')" class="iconfont reg-header-icon">
              &#xe6d5;
            </view>
          ...
          <view>
            <view :class="{
              ...
              <view v-if="titleStatus.headerBgIcon.includes('#xe6d0')" class="iconfont reg-header-icon-bg">
                &#xe6d0;
              </view>

              <view v-else-if="titleStatus.headerBgIcon.includes('#xe6de')" class="iconfont reg-header-icon-bg">
                &#xe6de;
              </view>

              <view v-else-if="titleStatus.headerBgIcon.includes('#xe6d5')" class="iconfont reg-header-icon-bg">
                &#xe6d5;
              </view>
            ...
            <view v-if="orderRegInfo.orderStatus === '10'" class="out-time-info f28 color-error">
        ...
        <view class="container">
          <view class="container-box g-border container-box1">
            <view v-if="isShowQr" class="qr-code g-flex-rc-cc g-border-bottom m32 flex1">
              ...
              <view class="qr g-flex-rc-cc flex1">
                <image v-if="showQrCode" :src="qrCodeOpt._qrImg" class="qrcode-img" />

                <image v-if="!showQrCode" :src="qrCodeOpt._barImg" mode="widthFix" class="barcode-img flex1" />
              </view>
            ...
            <view v-if="hosInfo.gisLat" @click="openHosLocation" class="hos-navigation g-flex-rc-cc m32 f32">
              <view class="hos-info">
            ...
            <view class="order-info">
              <g-form :value="orderRegInfo" forShowBodyAlign="left" bodyBold hideRowBorder ref="refForm">
                <template #showBody="{ item, value }">
                  <view @click="goDoctorCard" v-if="item.key === 'docName' &&
                  ...
                  <view v-else-if="item.key === '_category'" class="flex-normal doc-name">
                    <view>
          ...
          <view class="container-box order-patient g-border p32v">
            <g-form :value="orderRegInfo" forShowBodyAlign="left" hideRowBorder ref="refFormPatient" />
          </view>
    ...
    <view v-if="orderRegInfo.patientId && isShowFooter" class="footer g-border-top">
      <view @click="goHome" class="home g-flex-rc-cc">
        <view class="iconfont home-icon">&#xe6df;</view>
        <view>首页</view>
      </view>

      <button v-if="orderRegInfo.orderStatus === '0'" @click="refoundOrder" class="btn btn-plain btn-error g-border">
      ...
      <block v-if="orderRegInfo.orderStatus === '70'">
        ...
        <button v-if="orderRegInfo.rateFlag !== 1 && orderConfig.isOpenComment === '1'
          " @click="goRatePage" class="btn g-border btn-primary">
      ...
      <button v-if="['20', '23', '42', '43', '45'].includes(orderRegInfo.orderStatus)" class="btn g-border btn-primary"
        @click="againOrder">
        再次预约
      </button>
    </view>

    <xy-dialog title="" :content="dialogContent" :show="isCancelOrderDialogShow"
      @cancelButton="isCancelOrderDialogShow = false" @confirmButton="_cancelOrderDialogConfirm" />

    <g-pay :list="refPayList" :autoPayArg="payArg" @pay-click="getPayInfo" autoInOne ref="refPay">
      <view class="p32">
...
let init = async () => {
  ...
  result._appointmentDate = `${result.appointmentDate} ${result.ampmName + result.appointmentTime
    }`;
...
.reg-detail {
  ...
  .box {
    ...
    .container-box {
      ...
      .hos-navigation {
        background: url($base-url + 'reg-detail-position-bg.png') 100%/100% no-repeat;
        height: 144rpx;
