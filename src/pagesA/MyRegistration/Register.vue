...
const pagesList = {
  ...
  "2":
    '/pagesC/cloudHospital/myPath?path=pagesC/selfService/nucleicBilling&query=["token","openId"]', //核酸开单多院区
};
...
const hosLvs = computed(() => {
  const lvs = [...new Set(hosList.value.map((o) => o.hosLevelName))].map((o) => ({
    label: o,
    value: o,
  }));
...
const itemClick = (item: IHosInfo) => {
  ...
  if (isMedCopy.value) {
    const _idx = medCopyConfigList.value.findIndex((o) => o.hosId === item.hosId);
...
const hosSortChange = ({ item }) => {
  if (item === "按距离排序") {
    if (!isAuthLocation.value) {
      gStores.messageStore.showMessage("未授权位置信息， 不支持按距离排序", 3000, {
        closeCallBack() {
...
const getList = async (isRequestApi: boolean = true) => {
  ...
  if (getTypeNow.value === "病案复印") {
    ...
    medCopyConfigList.value = await ServerStaticData.getSystemConfig("medRecord");
  }
