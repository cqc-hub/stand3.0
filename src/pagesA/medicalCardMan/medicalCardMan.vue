<template>
  ...
  >
    ...
    <view class="pat-box">
      <view v-if="!isShowHealthCardMode" class="add-pat-box" @click="addPatPage">
      ...
      >
        <template #footer="{ pat }">
          ...
          <block v-if="$global.systemInfo.isOpenHealthCard && !pat.healthQrCodeText">
            <view v-if="!isShowHealthLogin" @click="upToHealthCord(pat)" class="jkk">
              升级为电子健康卡
            </view>
...
const addPatPage = () => {
  uni.navigateTo({
    url: globalGl.addPersonUrl + '?_directUrl=' + encodeURIComponent('/pages/home/home'),
  });
};
