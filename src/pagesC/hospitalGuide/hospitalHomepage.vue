<template>
  ...
  <view class="page" v-if="dataList">
    <scroll-view class="scroll-view" scroll-y="true" :scroll-into-view="toView" scroll-with-animation="true">
      <J-skeleton :loading="isLoading" :imgTitle="true" avatarSize="52px" titleWidth="100%" :row="row">
        <swiper
          class="swiper"
          :autoplay="true"
          circular
        ...
        >
          <swiper-item class="bg_image" id="hosIntroduce" v-for="item in dataList.bannerList" :key="item.id">
            <image mode="widthFix" :src="item.iconfont" @click="gotoPath(item)" />
          </swiper-item>
        </swiper>
      </J-skeleton>
      ...
      <view class="page-content">
        <view>
          <J-skeleton :loading="isLoading" :imgTitle="true" avatarSize="32px" titleWidth="100%" :row="row">
            <view class="card">
        ...
        <view class="hosButton">
          <J-skeleton :loading="isLoading" :imgTitle="true" :imgHeight="70" :row="row">
            <view class="hosBar">
              <view class="hosBarItem" @tap="useCommonTo(item)" v-for="item in gridBar" :key="item.id">
          ...
          <view class="mt16" v-if="!isLoading">
            <!-- <J-skeleton
              :loading="isLoading"
              :showAvatar="false"
              :showTitle="true"
              ...
              :row="row"
            > -->
              <view class="main-text" :class="isMore ? 'isMaxHeight' : ''">
                <image class="text-top" :src="$global.BASE_IMG + 'img_yyjs@3x.png'" />
                <text class="details">
                  {{ dataList.intro }}
                </text>
                <view v-if="isMore" class="more flex-normal" @tap="gotoGuide">
...
const gridBar = [
  ...
  },
  {
    ...
    terminalType: "my-h5",
    gridLabel: "0",
    path: "/pagesC/periodArchives/periodArchives",
...
const gridList = [
  ...
  },
  {
    ...
    query:
      '{   "_type":"useTBanner",   "type":"h5",   "isSelfH5":"1",   "path":"pagesC/healthRecord/healthRecordHis",   "text":"健康档案",   "addition":{     "patientId":"_patientId"   } }',
...
onLoad(async (p) => {
  ...
  hosId.value = pageProps.value.hosId && cacheStore.changeHosId(pageProps.value.hosId);
...
//   const useToPath = (item) => {
//     console.log('useToPath', item);
//     if (!gStores.globalStore.isLogin&& item.loginInterception == 1) {
//       uni.showToast({
//         title: '未登录，请先登录',
//         icon: 'none',
//       });
...
//       }, 1000);
//     } else {
//       if (item.terminalType === 'my-h5') {
//         uni.navigateTo({
//           url: joinQuery(item.path, {
//             hosId: hosId.value,
//           }),
//         });
//       } else if (item.terminalType === 'my') {
//         if (!item.path.includes('?')) {
//           goBackProgram(`${item.path}?hosId=${hosId.value}`, { type: 1 });
//         } else {
//           goBackProgram(`${item.path}&hosId=${hosId.value}`, { type: 1 });
//         }
//       } else if (item.terminalType === 'h5') {
...
//     title: res.result.aliasName,
//   });
  try {
    dataList.value.bannerList = JSON.parse(dataList.value.bannerJson);

    if (dataList.value.bannerList && dataList.value.bannerList.length > 0) {
    } else {
      throw new Error();
    }
  } catch {
    dataList.value.bannerList = [
      {
        iconfont: dataList.value.hosPhoto,
      },
    ];
  }
  setTimeout(() => {
