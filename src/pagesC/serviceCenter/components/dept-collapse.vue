  ...
  >
    <view
      ...
      :class="{
        ...
        'my-row-active':
          !item.children &&
          activeLv2?.filter((item1) => {
            return item?.hosDeptId == item1.hosDeptId;
          }).length > 0,
    ...
    >
      <view v-if="item.children" id="content" class="content">
        <view
          ...
          :class="{
            'my-row-active':
              activeLv3?.filter((item) => {
                return item?.hosDeptId == _item.hosDeptId;
              }).length > 0,
          }"
          class="my-row my-row-collapse"
        >
          {{ _item.deptName }}
          <view v-if="activeLv3?.filter((item) => {
                return item?.hosDeptId == _item.hosDeptId;
              }).length > 0" class="iconfont ico-check">&#xe6cc;</view>
        </view>
      </view>
    </view>
  </view>
