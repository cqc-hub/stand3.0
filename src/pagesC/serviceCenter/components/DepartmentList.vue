<template>
  <view class="container">
    ...
    <scroll-view v-if="isLv2" class="dept-list-lv2-scrollContainer" scroll-y>
      <view
        class="customize input-line 28"
        v-if="deptListLv1 && deptListLv1.firstHosDeptId === list[list.length - 1].firstHosDeptId"
      >
        <view class="input">
          <input
        ...
        <view class="self f28" @click.stop="handleSelf" style="color: #fff">
          <text :class="`iconfont icon-write`" style="color: #fff">
            &#xe6b9;
          </text>
          确认
        </view>
...
<script lang="ts" setup>
  ...
  import DeptCollapse from './dept-collapse.vue';
  ...
  const deptListLv1 = ref<IDeptLv1>();
  ...
  const handleSelf = () => {
    ...
    emits('handleCustomize', selfInput.value);
    selfInput.value = '';
  };
  ...
  onMounted(async () => {
    const defaultChoose =
      props.list.find((o) => o.firstDefaultShowDept === '1') || props.list[0];
    ...
    async () => {
      if (props.list.length) {
        if (props.level === '1') {
          ...
          if (activeLV1.value) {
            itemClickLv1(props.list[activeLV1.value]);
          }
          // const defaultChoose =
