  ...
  <view class="item f32">
    ...
    <view v-if="isChineseMedical(item)">
      ...
      <view v-if="detailData.drugDetailList" class="medical-content">
        <view v-for="(citem, ci) in detailData.drugDetailList" :key="ci" class="medical-item f28 color-444">
    ...
    <view v-else>
      <view v-if="detailData.drugDetailList" class="medical-content1">
        <view v-for="(citem, ci) in detailData.drugDetailList" :key="ci" :class="{
          'g-border-bottom': ci !== detailData.drugDetailList.length - 1,
        }" class="medical-item1 f28 color-444 p24v">
          <view class="flex-between mb8">
            <view class="g-bold flex-start">
              <text>{{ citem.drugName }}</text>
              <view class="medical-type" v-if="citem.isTake"><text>检查带药</text></view>
            </view>
