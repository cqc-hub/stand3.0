<template>
  <view class="page">
    ...
    <g-message />
    <view class="tab-box">
      <g-tabs v-model:value="tabCurrent" :tabs="tabs" :line-scale="0.8" field="headerName" all-blod @change="(e) => tabChange(e,'click')" />
    </view>

    <swiper v-if="tabs.length>0" :current="tabCurrent" @change="(e) => tabChange(e.detail.current,'')" class="container">
      <swiper-item v-for="tab in tabs" :key="tab.typeId">
        <view class="container-scroll">
          <scroll-list :option="scrollOption" @refresh="refresh" @load="load" ref="slist">
            <template #default>
              <view class="list-block">
                <template v-for="(item, index) in pageList[tab.typeId]" :key="index">
