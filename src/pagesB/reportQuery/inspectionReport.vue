<template>
  <view class="page" scroll-y="true">
    ...
    <view class="top" :class="{ bgc: top > 0 }" v-if="btnNumber && btnNumber > 1">
    ...
    <view class="container" :class="{ isShowTop: btnNumber && btnNumber > 1 }">
      <template v-for="(item, index) in examineReportList.detailsResult" :key="index">
        <view
        ...
        >
          <view class="container-block-top">
            <view class="container-top-click" @click="more(index)">
              ...
              <view class="patient-information">
                <view class="subhead"
                  >患者信息<view class="subhead-detail">
                    {{ nameConvert(pat.patientName) }}({{
                      ...
                      examineReportList.age || pat.patientAge
                    }}岁</view
                  >
                </view>
                <view class="subhead"
                ...
                <view class="hidden-patient-information" v-show="isShow[index] == 1">
                  <view class="subhead" v-if="item.regTime"
                    >采集时间<view class="subhead-detail">{{ item.regTime }}</view>
                  </view>
                  <view class="subhead"
                    >报告时间<view class="subhead-detail">{{ item.repTime }}</view>
                  </view>
                  <view class="subhead" v-if="item.serialNo"
                    >申请单号<view class="subhead-detail">{{ item.serialNo }}</view>
                  </view>
                  <view class="subhead"
                    >申请医生<view class="subhead-detail">{{ item.applyDoc }}</view>
                  </view>
                  <view class="subhead"
                    >报告医生<view class="subhead-detail">{{ item.reportDoc }}</view>
                  </view>
                  <view class="subhead"
            ...
            <view class="button-list">
              ...
              >
                <view class="icon-font ico_sy_paper1"></view> 图文报告
              </button>
              <button class="button" v-if="item.dicomList">
                <view class="icon-font ico_cloud"></view> 云影像
              </button>
            </view>
          </view>
          <view class="container-block-bottom">
            ...
            <view class="seen" v-if="item.impression || examineReportList.diagnosis">
    ...
    uni
      ...
      .boundingClientRect((data) => {
        ...
        for (var i = 0; i < domData.value.length; i++) {
          if (boxTop.value[i].top <= top.value && top.value <= boxTop.value[i].bottom) {
            chooseBtn.value = i;
            toView.value = "item" + i;
            return;
          }
...
const forShare = () => {
  const data = `${examineReportList.value.patientName}的检查报告,分享链接有效期至${
...
const actionItemClick = ({ item }) => {
  downloadRepId.value = encodeURIComponent(encryptDes(item.repId + "", "phsDesKey"));
