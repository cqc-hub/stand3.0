<template>
  <view
    ...
    class="g-page"
  >
  <view v-if="_allDocList.length&&isComplete" class="mt24 ml16 flex items-center justify-between"> 
    <view class="department-doc-title g-bold f36">
          名医列表({{ _allDocList.length }})
        </view>
  ...
  <view
    ...
    <view v-else-if="isComplete && list.length===0" class="empty-list">
      <g-empty :current="1" />
    </view>
    <g-massage />
  </view>
...
const getList = async () => {
  ...
  list.value = [];
  const { result = [] } = await api.getDoctorByDeptAndHos({
    famousDoctorType,
    hosId,
    hosDeptId,
  }).finally(() => {
