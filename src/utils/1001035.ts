...
export const useLunchInit1001035 = ({ onShow }) => {
  ...
  if (isOpenSdk) {
    try {
      ...
      const {
        initCtx1: initCtx,
      } = require('./libshadowesm1001035/shadow-init.js');
      shadowinit = initCtx;
    } catch (error) {
      console.error('加载省中加密模块失败:', error);
    }
  }

  const appLaunchInit1001035 = () => {
    if (isOpenSdk && shadowinit) {
      shadowinit.initShadowSDK();
    }
