...
type GreaterThan<T extends number, U extends number, R extends any[] = []> = T extends R['length']
  ? false
  : U extends R['length']
  ? true
  : GreaterThan<T, U, [...R, 1]>;
...
const getWeChatBase64ImageByUrl = function (url: string): Promise<TChooseImgBase64Res> {
...
const checkFileSize = ({ filePath, fileSize }: { filePath: string; fileSize: number }) => {
    ...
    uni.getFileInfo({
      ...
      complete: (res) => {
        ...
        if (size < fileSize) {
          ...
          const str = `请选择小于 ${(fileSize / 1024 / 1024).toFixed(1)}M 的相片`;
...
type OcrFindRes = ReturnType<typeof findSuccess>;
export type OcrRes = Record<keyof OcrFindRes, string>;

export const useOcr = async (): Promise<OcrRes> => {
  const messageStore = useMessageStore();
