...
const findSuccess = async ({
  ...
  return {
    idCardOcrEn,
    patientNameOcrEn,
    image: image?.startsWith('http') ? image : `data:image/jpg;base64,${image}`,
    name,
    sex,
    nation,
    birth,
...
export const useOcr = async (
  ...
  if (env === 'wx') {
    ...
    if (aliThroughByEnd && globalGl.SYS_CODE === '1001067') {
      // https://opendocs.alipay.com/open/05urve 纯服务端 收费
