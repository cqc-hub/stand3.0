...
export class LoginUtils extends GStores {
  ...
  async onAfterLoginAndBeforeGetPatList() {
    ...
    if (['1001086', '1001094'].includes(sysCode) && ev === 'wx') {
      ...
      api.xjzyyPatSync(reqData);
    }
    if (['1001083'].includes(sysCode)) {
      api.wzrmPatSync({ source: this.globalStore.browser.source });
    }

    //通过手机号同步
    if (['1001095'].includes(sysCode)) {
      api.patSync({ source: this.globalStore.browser.source });
    }
