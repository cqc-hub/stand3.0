class IdValidator {
	...
	getIdCardInfo(idCardNo) {
		...
		const _age = this.calculationAge(idCardInfo.birthday);
		idCardInfo.age = (_age && _age) || 0;
		return idCardInfo;
	}
...
/**
 * if(idValidator.checkIdCardNo(idCard)) {
 *  idValidator.getIdCardInfo(idCard)
 * }
 */
