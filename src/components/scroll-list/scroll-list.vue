...
export default {
		...
		scrollViewObserve() {
			...
			this.$nextTick(() => {
				this.queryRect('.' + this.elClass).then((rect) => {
					...
					scrollViewObserve.observe(
						'.' + this.elClass,
						(position) => {
							// #ifdef H5
							this.scrollViewTop =
								position.boundingClientRect.top -
								this.windowInfo.windowTop;
							// #endif
							// #ifndef H5
							this.scrollViewTop =
								position.boundingClientRect.top;
	...
	computed: {
		...
		listWrapStyle() {
			...
			style.height =
				this.scrollViewHeight - uni.upx2px(offsetBottom) + 'px';
		...
		pullDownAnimationStyle() {
			const style = {};
			const { refresherIconColor, refresherThreshold } =
				this.defaultOption;
